        ...
        table.on('tool(demo)', function(obj) {
            ...
            if (layEvent === 'detail') { //查看
                ...
                layer.open({
                    type: 2
                    ,title: '编辑'
                    ,content: 'Editor'+page+'.html' //展示.html中的layui表单
                    ,maxmin: true   //允许最大化
                    ...
                    ,yes: function(index, layero) {
                        //通过以下的方法获取回调的数值
                        var patient_id = $(layero).find('iframe')[0].contentWindow.patient_id.value;
                        var patient_name = $(layero).find('iframe')[0].contentWindow.patient_name.value;
                        var patient_gender = $(layero).find('iframe')[0].contentWindow.patient_gender.value;
                        var patient_date_start = $(layero).find('iframe')[0].contentWindow.patient_date_start.value;
                        var patient_dept = $(layero).find('iframe')[0].contentWindow.patient_dept.value;
                        var patient_area = $(layero).find('iframe')[0].contentWindow.patient_area.value;
                        var patient_doc_id = $(layero).find('iframe')[0].contentWindow.patient_doc_id.value;
                        var patient_age = $(layero).find('iframe')[0].contentWindow.patient_age.value;
                        var patient_condition = $(layero).find('iframe')[0].contentWindow.patient_condition.value;
                        var patient_phone = $(layero).find('iframe')[0].contentWindow.patient_phone.value;
                    ...
                    ,success: function(layero, index){
                        ...
                        body.find("#patient_id").val(data.patient_id);
                        body.find("#patient_name").val(data.patient_name)
                        body.find("#patient_gender").val(data.patient_gender)
                        body.find("#patient_date_start").val(data.patient_date_start)
                        body.find("#patient_dept").val(data.patient_dept)
                        body.find("#patient_area").val(data.patient_area)
                        body.find("#patient_doc_id").val(data.patient_doc_id)
                        body.find("#patient_age").val(data.patient_age)
                        body.find("#patient_condition").val(data.patient_condition)
                        body.find("#patient_phone").val(data.patient_phone)
